...
const PersonForm = () => {
    ...
    const handleSubmit = (e) => {
        ...
        // Deletes property "_id" of a fetched person to comply with API requirements
        if (person._id) console.log(delete person._id);

        (id ? apiPut("/api/persons/" + id, person) : apiPost("/api/persons", person))
            .then(() => {
                setSent(true);
                setTimeout(() => {
                navigate("/persons")}, 3000);
            })
            .catch((error) => {
